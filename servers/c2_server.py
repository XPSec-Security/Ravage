...
class C2Server:
    ...
    def _parse_configured_uris(self):
        agent_uris = self.config_loader.get_agent_uris()
        parsed_uris = []
    ...
    def _validate_agent_headers(self):
        expected_user_agent = self.config_loader.get_agent_user_agent()
        expected_hosts = self.config_loader.get_agent_upstream_hosts()
    ...
    def _validate_request_headers(self):
        expected_headers = self.config_loader.get_agent_request_headers()
    ...
    def _apply_agent_headers(self, response):
        agent_headers = self.config_loader.get_agent_response_headers()
    ...
    def _handle_agent_delivery(self):
        try:
            agent_content = self.agent_generator.generate_agent()
    ...
    def run(self):
        profile_config = self.config_loader.get_agent_profile_config()
        bind_config = profile_config.get('bind', {})

        host = bind_config.get('host', '0.0.0.0')
        port = bind_config.get('port', 80)
